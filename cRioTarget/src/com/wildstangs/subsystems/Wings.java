...
public class Wings extends WsSubsystem implements IObserver
{
    ...
    public void update()
    {
        ...
        (WsOutputManager.getInstance().getOutput(WsOutputManager.WINGS_SOLENOID_1_INDEX)).set((IOutputEnum) null, new Integer(wingsValue));
        (WsOutputManager.getInstance().getOutput(WsOutputManager.WINGS_SOLENOID_2_INDEX)).set((IOutputEnum) null, new Integer(wingsValue));
